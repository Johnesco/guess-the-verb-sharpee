...
class GameEngine {
    ...
    introspect() {
        ...
        const capsByTrait = new Map();
        for (const [key] of (0, world_model_1.getAllCapabilityBindings)()) {
            const [traitType, capability] = key.split(':');
            const list = capsByTrait.get(traitType) ?? [];
            list.push(capability);
            capsByTrait.set(traitType, list);
        }
        const intsByTrait = new Map();
        for (const [key] of (0, world_model_1.getAllInterceptorBindings)()) {
        ...
        const behaviors = [];
        for (const [key, binding] of (0, world_model_1.getAllCapabilityBindings)()) {
            const [traitType, capability] = key.split(':');
            const behavior = binding.behavior;
            const phases = [];
            if (typeof behavior.validate === 'function')
                phases.push('validate');
            if (typeof behavior.execute === 'function')
                phases.push('execute');
            if (typeof behavior.report === 'function')
                phases.push('report');
            if (typeof behavior.blocked === 'function')
                phases.push('blocked');
            behaviors.push({
                traitType,
                actionId: capability,
        ...
        for (const [key, binding] of (0, world_model_1.getAllInterceptorBindings)()) {
            const [traitType, actionId] = key.split(':');
            const interceptor = binding.interceptor;
            const phases = [];
            if (typeof interceptor.preValidate === 'function')
                phases.push('preValidate');
            if (typeof interceptor.postValidate === 'function')
                phases.push('postValidate');
            if (typeof interceptor.postExecute === 'function')
                phases.push('postExecute');
            behaviors.push({
                traitType,
                actionId,
                priority: binding.priority ?? 0,
        ...
        return { actions, traits, behaviors };
    }
    /**
