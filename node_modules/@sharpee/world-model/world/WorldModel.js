...
const regionTrait_1 = require("../traits/region/regionTrait");
const sceneTrait_1 = require("../traits/scene/sceneTrait");
...
class WorldModel {
    ...
    createRegion(id, options) {
        ...
        const entity = new if_entity_1.IFEntity(id, entity_types_1.EntityType.REGION, {
            attributes: {
                displayName: options.name,
                name: options.name,
                entityType: entity_types_1.EntityType.REGION,
            },
        });
        entity.add(new regionTrait_1.RegionTrait(options));
        this.entities.set(id, entity);
        return entity;
    }
    /**
    ...
    assignRoom(roomId, regionId) {
        ...
        const roomTrait = room.get(trait_types_1.TraitType.ROOM);
        if (!roomTrait) {
        ...
        if (!region.hasTrait(trait_types_1.TraitType.REGION)) {
    ...
    isInRegion(entityId, regionId) {
        ...
        let roomRegionId;
        if (entity.type === entity_types_1.EntityType.ROOM) {
            roomRegionId = entity.get(trait_types_1.TraitType.ROOM)?.regionId;
        }
        else {
            const room = this.getContainingRoom(entityId);
            if (!room)
                return false;
            roomRegionId = room.get(trait_types_1.TraitType.ROOM)?.regionId;
        }
        if (!roomRegionId)
            return false;
     ...
     */
    getRegionCrossings(fromRoomId, toRoomId) {
        const fromRoom = this.getEntity(fromRoomId);
        const toRoom = this.getEntity(toRoomId);
        const fromChain = fromRoom
            ? this.getRegionAncestry(fromRoom.get(trait_types_1.TraitType.ROOM)?.regionId)
            : [];
        const toChain = toRoom
            ? this.getRegionAncestry(toRoom.get(trait_types_1.TraitType.ROOM)?.regionId)
            : [];
    ...
    getRegionAncestry(regionId) {
        ...
        while (currentId) {
            ...
            if (!region)
                break;
            const trait = region.get(trait_types_1.TraitType.REGION);
            if (!trait)
                break;
     ...
     */
    regionAncestryIncludes(startRegionId, targetRegionId) {
        let currentId = startRegionId;
        const visited = new Set();
        while (currentId) {
            if (currentId === targetRegionId)
                return true;
            if (visited.has(currentId))
                return false;
            visited.add(currentId);
    ...
    createScene(id, options) {
        ...
        const entity = new if_entity_1.IFEntity(id, entity_types_1.EntityType.SCENE, {
            attributes: {
                displayName: options.name,
                name: options.name,
                entityType: entity_types_1.EntityType.SCENE,
            },
        });
        entity.add(new sceneTrait_1.SceneTrait({
            name: options.name,
    ...
    isSceneActive(sceneId) {
        ...
        if (!entity)
            return false;
        return entity.get(trait_types_1.TraitType.SCENE)?.state === 'active';
    }
    /**
     ...
     * @param sceneId - The scene entity ID.
     */
    hasSceneEnded(sceneId) {
        const entity = this.getEntity(sceneId);
        if (!entity)
            return false;
        return entity.get(trait_types_1.TraitType.SCENE)?.state === 'ended';
    }
    /**
     ...
     * @param sceneId - The scene entity ID.
     */
    hasSceneHappened(sceneId) {
        const entity = this.getEntity(sceneId);
        if (!entity)
            return false;
        return entity.get(trait_types_1.TraitType.SCENE)?.beganAtTurn !== undefined;
    }
}
